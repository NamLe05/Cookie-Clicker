...
public class CookieClickerMenu implements ActionListener {
    ...
    public void background() throws FileNotFoundException {
        while (mainPanel.isVisible()) {
            ...
            if (start == true) {
                ...
                new GUI(0,0,0,0);
            } else if (load == true) { // if clicked load button
                load = getData();
                if (load == true) { // if savefile exists
                    mainFrame.setVisible(false); // create new GUI object with saved information
    ...
    private boolean getData() throws FileNotFoundException{
        ...
        if(saveFile.exists()) {
            try (// scanner reads the contents of the file
            Scanner readFile = new Scanner(saveFile)) {
                ...
                savedBakeries = readFile.nextInt(); // assigns values
            }
            return true;
        } else {
